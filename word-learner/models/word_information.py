...
class WordInformation(SQLModel, table=True):
    ...
    # phonetic_spelling: str = Field(description="American non-phonemic pronunciation respelling of
    # the word what Google provides in small letters")
    english_meaning: str = Field(
        description="Meaning of the word in English. List as many word classes as possible."
    ...
    japanese_meaning: str = Field(
        ...
        " Add the prefix [<word_class>]: and add '|' at the end of each word class"
        " if there are following word classes."
        " For example, '[名詞]: 援助者, 手助けとなる人または物|[形容詞]: 補助の, 補助的な'"
    )
    japanese_example: str = Field(description="translation of the English example in Japanese")
    japanese_example2: str = Field(description="translation of the English example 2 in Japanese")
